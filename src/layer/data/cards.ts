...
import type {Content} from "@coorpacademy/types/progression-engine";
...
const computeCardCompletionRate = (levels: Array<CardLevel>): number =>
    levels.reduce(
        (accumulator, currentValue) => accumulator + currentValue.completion,
        0,
    ) / levels.length;

const cardToCompletion = (
    card: DisciplineCard | ChapterCard | CardLevel,
): Completion => ({
    stars: card.stars,
...
export const updateDisciplineCardDependingOnCompletion = (
    ...
    const levelCards = card.modules.map((levelCard, index) => {
        ...
        const levelCompletion = mergeCompletion(
            cardCompletion,
            latestCompletion,
        );
    ...
    const stars = levelCards.reduce(
        (acc, levelCard) => acc + levelCard.stars,
        0,
    );
...
const refreshDisciplineCard = async (
        ...
        disciplineCard.modules.map(
            async (level): Promise<Completion | null> => {
                const completionKey = buildCompletionKey(
                    ENGINE.LEARNER,
                    level.universalRef || level.ref,
                );
                const completionString = await AsyncStorage.getItem(
                    completionKey,
                );
    ...
    return updateDisciplineCardDependingOnCompletion(
        latestCompletions,
        disciplineCard,
    );
};

const refreshChapterCard = async (
    chapterCard: ChapterCard,
): Promise<ChapterCard> => {
    ...
    const mergedCompletion = mergeCompletion(
        cardCompletion,
        JSON.parse(latestCompletion),
    );
    return updateChapterCardAccordingToCompletion(
        mergedCompletion,
        chapterCard,
    );
};
...
const createDisciplineCardForModules = (
    card: DisciplineCard,
...
export const fetchCard = async (
    ...
    if (__E2E__) {
        ...
        cards = createDisciplinesCards(disciplines)
            ...
            .filter(card => {
                return (
                    ...
                    (card.modules &&
                        card.modules.find(
                            mod => mod.universalRef === content.ref,
                        ))
                );
            });
    } else {
        ...
        const response = await fetch(
            `${jwt.host}/api/v2/contents?${buildUrlQueryParams(query)}`,
            {
...
export const fetchCards = async (
    ...
    if (__E2E__) {
        ...
        const _cards = createDisciplinesCards(disciplines).concat(
        ...
        const response = await fetch(
            `${host}${endpoint}?${buildUrlQueryParams(query)}`,
            {
                headers: {authorization: token},
            },
        );
