...
const attemptToRetrieveContent = async (
    ...
    if (!token || !brand) {
        throw new Error(
            "Chapter progression not created - no token or brand provided",
        );
    }
    ...
    const contentId =
        card.type === CARD_TYPE.COURSE && levelRef
            ? levelRef
            : card.universalRef;
...
export const selectCard = (
    ...
    try {
        ...
        if (item.type === CARD_TYPE.COURSE) {
            ...
            if (!level) {
                level = await attemptToRetrieveContent(
                    item,
                    state,
                    services,
                    nextLevel.universalRef,
